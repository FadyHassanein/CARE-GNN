...
\begin{frame}{Headline Result (YelpChi)}
    ...
    \vspace{0.8em}
    \begin{center}\normalsize
    \textbf{+1.99\,pp} AUC over our CARE-GNN reproduction\\
    \textbf{+2.86\,pp} AUC over Dou~et~al.~(2020)'s published CARE-GNN\\[0.3em]
...
\begin{frame}{Comparison with the CARE-GNN Paper}
    ...
    \vspace{0.5em}
    \footnotesize
    Our full framework improves AUC by \textbf{+2.86\,pp} over the paper-reported
    CARE-GNN baseline. Recall is slightly lower, consistent with the AUC--recall
    trade-off of a better-ranking classifier at the same decision threshold.
\end{frame}
