...
This chapter reports the experimental evaluation of the unified GNN + RL + LLM framework proposed in Chapter~3. The main thesis claim---that the three-pillar combination significantly outperforms CARE-GNN on a challenging real-world review graph---is tested on the YelpChi dataset in Section~\ref{sec:results-unified}. Supporting evidence for the internal design decisions is provided by the Amazon component ablations (A1--A9), which isolate individual pillars and sub-components in a simpler setting. All experiments use the best validation-AUC checkpoint over 31 training epochs. The primary evaluation metrics are AUC-ROC and Average Precision (AP)---threshold-independent ranking metrics most appropriate for the highly imbalanced fraud detection setting. F1, Precision, and Recall are reported as secondary metrics. Where possible we report the mean $\pm$ standard deviation across three random seeds.

\paragraph{Headline result.} On YelpChi (3 seeds, 31 epochs each) the unified GNN + RL + LLM framework achieves AUC $0.7856 \pm 0.0022$, AP $0.4108 \pm 0.0052$, and F1-macro $0.6099 \pm 0.0565$, improving on our CARE-GNN reproduction ($0.7657 \pm 0.0007$ AUC) by $+1.99$pp with Welch's $t$-test $p < 0.001$. The detailed table and analysis are given in Section~\ref{sec:results-unified}; readers interested primarily in the main contribution may skip directly to that section.
...
\subsection{Supporting Evidence: Baseline Models on Amazon (GCN, GraphSAGE)}
...
\subsection{Supporting Evidence: GNN + RL Pillars on Amazon (A1)}
...
\subsection{Supporting Evidence: Component Ablations on Amazon (A2--A9)}
...
\subsubsection{A8: Feature-Level Enrichment}
...
\subsubsection{A9: Combined LLM Components}
...
\subsection{Summary of Amazon Component Ablations}
...
\subsection{Key Findings from Amazon Component Ablations}
...
The Amazon experiments (A1--A9) evaluated CAPN using handcrafted features and LLM-generated structural scores. A natural question is whether \emph{raw textual content}---the actual review text---can further improve fraud detection when integrated into the CAPN architecture. To investigate this, we conduct experiments on the YelpChi dataset, where we recovered the original review text for all 45,954 nodes and generated sentence embeddings using \texttt{all-MiniLM-L6-v2}.
...
Table~\ref{tab:paper-comparison} compares our CARE-GNN reproduction with the results reported in the original paper \cite{dou2020enhancing} (Table~3, 40\% training data).
...
\bottomrule
\end{tabular}
\caption{CARE-GNN reproduction comparison with the original paper \cite{dou2020enhancing}.}
\label{tab:paper-comparison}
\end{table}

\noindent
Our Yelp reproduction (AUC 0.7660) is within 1pp of the paper's reported value (0.7570), with the minor difference attributable to different train/validation split ratios and random seeds. The Amazon reproduction exceeds the paper by +4.25pp, likely due to implementation improvements including cached feature loading and consistent L1 normalisation.
...
\bottomrule
\end{tabular}
\caption{Text embedding integration results on the Yelp dataset. Three progressive improvements: deeper projection (Phase~1), soft attention neighbour weighting (Phase~2), and advantage-normalised REINFORCE with entropy regularisation (Phase~3).}
...
The original hard top-$K$ filtering (threshold controls only neighbour count) gives the policy network a weak control lever. We replaced it with soft attention weighting, where the threshold acts as a temperature parameter: $w_i = \mathrm{softmax}(-d_i / \tau)$, allowing the policy to control \emph{how} neighbours are weighted, not just \emph{how many}. Result: no improvement (Val AUC 0.7738 vs.\ 0.7737).

\paragraph{Phase 3: Stronger policy learning.}
Vanilla REINFORCE with scalar batch reward is known to have high variance. We added: (1) advantage normalisation via running EMA of reward mean and standard deviation, (2) entropy regularisation to prevent threshold collapse, and (3) increased policy learning rate (1e-3$\to$3e-3) and loss weight ($\lambda_\pi$ 0.1$\to$0.3). Result: no improvement (Val AUC 0.7734 vs.\ 0.7738).
...
\begin{enumerate}
    ...
    \item \textbf{The policy network is the wrong injection point for content features.} Text embeddings feed into the policy state, which outputs neighbour filtering thresholds. This is an indirect path: text $\to$ projection $\to$ state $\to$ threshold $\to$ neighbour selection $\to$ aggregation $\to$ prediction. Even if text contained discriminative signal, it would need to improve \emph{neighbour selection decisions}, not \emph{classification} directly.

    \item \textbf{Contrast with the FLAG approach.} The FLAG framework \cite{yang2025flag} uses text differently: an LLM extracts \emph{discriminative text} specifically prompted to identify fraud-relevant patterns, and text is used for \emph{semantic similarity neighbour sampling} with end-to-end fine-tuning. Our approach (frozen sentence embeddings projected into a policy state) is fundamentally more limited.
\end{enumerate}

\paragraph{Architectural insight.} This experiment reveals a separation of concerns in GNN fraud detection: the policy network benefits from \emph{structural} features (degree, neighbour overlap, feature variance) that inform neighbour selection, while \emph{content} features (text semantics) are better suited for direct classification enrichment. This finding is consistent with the Amazon results, where LLM reasoning scores---compact, fraud-relevant assessments---improved performance through the RL state, but generic high-dimensional embeddings did not.

\subsection{Main Result: Unified GNN + RL + LLM Framework on YelpChi}
\label{sec:results-unified}

This section presents the \emph{primary result} of this thesis: the unified three-pillar framework evaluated on the more challenging YelpChi dataset. The preceding subsection (Yelp text embedding experiments) established that naive sentence-transformer enrichment fails on Yelp; that negative result motivates the specific design choices of the unified framework (fraud-specific text scores instead of generic embeddings, gated injection instead of concatenation, actor-critic policy optimisation instead of REINFORCE). The Amazon ablations (A1--A9) reported earlier validate the individual components in a simpler setting; the YelpChi evaluation reported here is the test that directly addresses the thesis claim---that the three pillars, combined, produce a statistically significant improvement over CARE-GNN.

\subsubsection{Experimental Protocol}

Each of the four rows defined in Section~\ref{sec:study-a10} is trained for $31$ epochs on three random seeds $\{72, 73, 74\}$ using the same data split and hyperparameters documented earlier in this chapter. For Rows~2 and~4, the GNN warmup is five epochs and the policy ramp is a further five epochs; the policy weight is $\lambda_\pi = 0.3$ (warmup-ramped), the critic weight is $\lambda_c = 0.1$, the critic learning rate is $10^{-3}$, and the Yelp priors file at \path{data/llm_priors/yelp_priors.json} seeds the policy-network relation biases and the multi-view distance mixing parameter $\gamma_r$. Text risk scores are computed in template mode from \path{data/yelp_review_texts.json} and cached at \path{llm_embeddings/yelp/text_risk_scores.pt}. No change is made to under-sampling, loss function, or evaluation code relative to the Amazon protocol.
...
\bottomrule
\end{tabular}
\caption{Main result -- Unified three-pillar framework on YelpChi, 3 seeds (72, 73, 74). Best value per column in bold.}
...
\begin{itemize}
    \item \textbf{Row 1 $\to$ Row 2 ($+0.24$pp).} The actor-critic CAPN policy with LLM priors replaces the heuristic RL module. The improvement is modest but extremely tight (variance drops from $\pm 0.0007$ to $\pm 0.0005$), confirming the stabilising effect of the learned value baseline.
    \item \textbf{Row 1 $\to$ Row 3 ($+1.40$pp).} The TextFeatureGate applied to node features---with the original heuristic RL unchanged---accounts for most of the gain. This is the LLM pillar's core contribution.
    \item \textbf{Row 3 $\to$ Row 4 ($+0.59$pp).} Adding actor-critic RL on top of the text gate yields the full framework. Variance also shrinks from $\pm 0.0038$ to $\pm 0.0022$, indicating the RL component not only raises the mean but regularises the LLM path.
\end{itemize}
...
\subsubsection{Comparison with the Original CARE-GNN Paper}

Dou et al.~\cite{dou2020enhancing} report two metrics for YelpChi at 40\% training data: AUC-ROC and Recall-macro. Table~\ref{tab:main-framework-paper-compare} places the unified framework alongside the paper's baselines at the comparable training fraction.
...
\bottomrule
\end{tabular}
\caption{YelpChi comparison with the original CARE-GNN paper~\cite{dou2020enhancing}, Table~3 (40\% training data). Starred rows are paper-reported values.}
\label{tab:main-framework-paper-compare}
\end{table}

\noindent
The unified framework improves AUC by $+2.86$pp over the paper-reported CARE-GNN baseline ($0.7570 \to 0.7856$). Recall is slightly lower than the paper's $0.7192$, consistent with the AUC/recall trade-off: a better-ranking classifier selects a marginally more conservative operating point at equal probability threshold. Because the paper reports a single-run maximum rather than a multi-seed mean, the appropriate comparison is against our Row 1 reproduction ($0.7657$), and the improvement attributable to the three-pillar framework is the $+1.99$pp we report.

\subsubsection{Evaluation Metrics}

We note two differences between our evaluation protocol and the CARE-GNN paper's:
\begin{enumerate}
    ...
    \item \textbf{Seed discipline.} The paper reports the best epoch of a single training run. We report the mean over three seeds with the checkpoint selected by validation AUC per run. This gives a lower but more honest estimate of deployable performance.
\end{enumerate}

\subsubsection{Findings}

The unified framework evaluation yields four findings:
...
\begin{enumerate}
    ...
    \item \textbf{The RL pillar remains necessary} even when LLM features are present ($+0.59$pp mean AUC from Row 3 to Row 4). Actor-critic policy optimisation uses the richer state vector (including textual scores) to produce better per-node filtering thresholds than the heuristic RL can achieve.
    \item \textbf{TextFeatureGate resolves the feature-level enrichment failure} observed on Amazon. Learnable sigmoid-gated injection preserves the node-feature dimensionality while letting training decide how much of the text signal to admit. The gate starts at a sigmoid value near $0.12$ so the initial network behaviour is close to CARE-GNN and the model is free to \emph{unlearn} the text contribution where it is unhelpful.
\end{enumerate}

\paragraph{Thesis position.} On the Amazon dataset the three pillars are individually near-redundant because 25 handcrafted features already yield AUC $\approx 0.94$. On the more challenging YelpChi dataset the same three pillars combine to deliver a statistically significant improvement over the published CARE-GNN baseline. This validates the design hypothesis: \emph{each pillar addresses a distinct failure mode, and the pillars compose additively when their integration interfaces (gated features, actor-critic advantage, compact state enrichment) are engineered to avoid information loss}. The unified framework is therefore the recommended configuration for heterogeneous review-graph fraud detection under realistic conditions where textual content is available.
