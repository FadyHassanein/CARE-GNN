...
\begin{enumerate}
    ...
    \item Three generality studies: the framework's semantic signal significantly improves six of eight external backbones---spanning linear models, tree ensembles, and relational and spectral GNNs, including the benchmark-leading XGB-Graph; the full framework attaches, as a wrapper with two localised changes, to a second selection-based backbone; and a scorer head-to-head shows a zero-cost heuristic matches the LLM on every graph backbone---the signal's value is in its six fraud-specific dimensions, not the scorer, so the pillar deploys without API dependence.
\end{enumerate}
...
\caption{The three-pillar framework, annotated with each pathway's held-out
contribution (Section~\ref{sec:results}). A multi-relation CARE-GNN backbone
(\emph{structural}) is filtered by the clean, neutrally-initialised CAPN policy
(\emph{adaptive}, $+0.58$~pp); six cached fraud-specific text scores---from an
LLM \emph{or} a zero-cost heuristic scorer, which perform indistinguishably on
graph backbones---enter as a gated residual into the node features
(\emph{semantic}, $+1.97$~pp). The dashed scores-to-policy-state channel and
the LLM-prior initialisation are part of the original design but are measured
to contribute nothing and to suppress the policy, respectively; the reported
framework ablates both.}
\label{fig:arch}
\end{figure}
...
\caption{The CAPN mechanism (clean configuration). A per-node state
$s_{v,r}$---node features, label-predictor confidence, and structural
statistics---is mapped by a per-relation Beta policy to a single threshold
...
inter-relation aggregation. The optional LLM scores $z_v$ in the state (dashed)
measure $\approx 0$ contribution, and the policy is initialised neutrally: the
LLM-prior initialisation of our original design suppresses it
...
the per-seed picture behind those averages: all twenty held-out seeds rise
almost in lockstep as the pillars are added, which is why the paired tests are
so decisive despite the modest absolute deltas.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{results_ablation}
...
happens on noisier hosts in Section~\ref{sec:wrapper}. We also isolated the
framework's second semantic-to-adaptive coupling---the LLM scores routed into
the policy state: removing them from the full configuration shifts AUC by an
insignificant $+0.33$~pp ($p=0.44$), i.e.\ this routing, too, contributes
nothing measurable; the framework's gains come from the gate-injected features
and the clean policy. The general lesson we draw: hand-designed couplings
between pillars can silently suppress or dilute the very components they are
meant to help, and only component-wise de-confounded ablation exposes this.
...
(BWGNN-Hetero $+0.62$). Figure~\ref{fig:doseresponse} plots this relationship
across all nine backbone configurations: the gain declines by roughly
$1.3$~pp for every $+0.1$ of backbone AUC, yet remains positive---and usually
significant---even at the strongest backbone we have. The framework's semantic
pillar is therefore not an architecture-bound component but a portable source
of fraud signal the behavioural features do not already contain---evidence
that the framework's strongest pathway generalises to any detector a
practitioner already runs.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{results_doseresponse}
\caption{Dose--response of the semantic signal. Each point is one backbone at
the identical frozen split: horizontal position is its AUC \emph{without}
text6, vertical position the AUC gain from adding text6 (filled = significant).
The signal helps most where the backbone is weakest and stays positive
everywhere; GraphConsis's large but non-significant point reflects that
backbone's high seed variance (Section~\ref{sec:wrapper}).}
...
A sharper question is whether this signal requires a language model at all. The
six score definitions are deliberately interpretable (templated phrasing,
sentiment--rating contradiction, copy-paste repetition), which means a
deterministic, zero-cost heuristic scorer can be written for them---and we
wrote one, then ran the head-to-head at the identical protocol
(Table~\ref{tab:headtohead}). The two scorers produce genuinely different
score vectors (mean $|\Delta| = 0.20$ on $[0,1]$; per-column correlation
$-0.16$ to $0.56$), yet on \emph{every graph backbone}---including our headline
CARE-GNN---their downstream AUC is statistically indistinguishable; the LLM
retains a small significant edge only on simple feature-only classifiers. The
semantic pillar's value therefore lies in \emph{which six quantities are
measured}, not in who computes them: for graph-based detectors the signal
costs nothing to produce, and the LLM is an optional scorer whose modest edge
surfaces only where no aggregation smooths it away.
...
The semantic pillar also has a structural boundary, and Amazon marks it. The
Amazon benchmark ships no review text, so the textual pathway cannot exist
there; only graph-derived LLM scores are available, and on Amazon's already
strong features they contribute essentially nothing---$+0.05$~pp AUC when
routed into the policy state, and $-0.59$~pp AUC ($-2.87$~pp F1) when injected
as node features. This is the dose--response pattern of
Table~\ref{tab:portable} taken to its limit: the semantic pillar helps where
text exists and behavioural features leave headroom, and adds nothing where
either condition fails. We therefore evaluate the textual pathway on YelpChi
and state that scope explicitly rather than leave it implicit.

We stress what Table~\ref{tab:portable} does \emph{not} claim. It does not show
that dropping these backbones into the full three-pillar framework yields these
gains---the adaptive pillar (CAPN) is bound to a neighbour-selection step and
was not applied here, and the injection for the non-CARE models is plain
concatenation rather than the gate. Table~\ref{tab:portable} isolates the
signal; the next section attaches the \emph{whole} framework to a second
backbone.
