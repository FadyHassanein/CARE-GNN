...
On the YelpChi benchmark (five seeds, validation-selected checkpoints), the full framework significantly outperforms its structural base: AUC $0.7838 \pm 0.0050$ ($+1.90$~pp over our CARE-GNN reproduction, $p<0.01$) on the standard backbone, and $0.7951 \pm 0.0032$ ($+2.94$~pp, $p<0.01$) on a capacity-upgraded variant---where the semantic pillar also \emph{stabilises} otherwise-divergent training. A pillar-wise ablation attributes the gain primarily to the semantic pillar ($+1.69$~pp, $p<0.001$); the adaptive pillar's solo contribution grows monotonically as the host's selection mechanism becomes richer ($+0.19\!\to\!+0.46\!\to\!+1.20$~pp across three backbones) but stays within seed noise, a result we report transparently. Two further studies establish the framework's generality: its semantic signal transfers to eight backbones at the identical label-scarce split, significantly improving six---including the benchmark-leading XGB-Graph (AUC $0.9106\!\to\!0.9177$)---and the whole framework attaches unchanged, as a backbone-agnostic wrapper, to GraphConsis, where its semantic pathway again delivers the largest gain ($+4.16$~pp) together with a $3.5\times$ variance reduction.
}
...
\begin{enumerate}
    ...
    \item The Camouflage-Aware Policy Network (CAPN), an actor--critic policy that replaces CARE-GNN's heuristic bandit with per-node, per-relation thresholds conditioned on a state vector fusing structural, label, and LLM-derived signals---together with an honest measurement of where, on a single-layer backbone, it does and does not add separable signal.
...
Table~\ref{tab:framework} builds the framework on the CARE-GNN backbone and
adds the pillars one at a time. The structural backbone alone (R1) reaches
$0.7648$ AUC. Adding the adaptive pillar (R2) moves it to $0.7667$---a $+0.19$~pp
change that does not clear the seed noise ($p=0.10$). Adding the semantic pillar
instead (R3) lifts AUC to $0.7817$, a $+1.69$~pp gain that is highly significant
($p<0.001$)---an effect we later show generalises far beyond this architecture
(Section~\ref{sec:generality}). The full framework (R4) reaches $0.7838$
AUC, $+1.90$~pp over the backbone ($p<0.01$); but stacking the adaptive pillar on
top of the semantic one buys a further $+0.21$~pp that again falls within noise
($p=0.30$).

\begin{table}[t]
\centering
\caption{Pillar-wise ablation on the CARE-GNN backbone, YelpChi $25/15/60$,
five seeds (mean $\pm$ sample std). The semantic pillar accounts for essentially
all of the framework's gain; the adaptive pillar's marginal contribution stays
within seed noise on this backbone.}
\label{tab:framework}
\begin{tabular}{lccc}
\toprule
Configuration & AUC & AP & F1-macro \\
\midrule
R1\; Structural (CARE-GNN)        & $0.7648 \pm 0.0020$ & 0.374 & 0.594 \\
R2\; \;+\,Adaptive (CAPN)         & $0.7667 \pm 0.0006$ & 0.380 & 0.596 \\
R3\; \;+\,Semantic (LLM)          & $0.7817 \pm 0.0026$ & 0.400 & 0.613 \\
R4\; Full framework               & $\mathbf{0.7838 \pm 0.0050}$ & \textbf{0.403} & 0.604 \\
\midrule
\multicolumn{4}{l}{\emph{Paired $t$-tests (test AUC, df${=}4$):}}\\
\multicolumn{4}{l}{Full vs.\ backbone: $+1.90$~pp, $p=0.002$\;$^{**}$}\\
\multicolumn{4}{l}{Semantic (R3$-$R1): $+1.69$~pp, $p{<}0.001$\;$^{***}$}\\
\multicolumn{4}{l}{Adaptive (R2$-$R1): $+0.19$~pp, $p=0.10$\;$^{\dagger}$}\\
\multicolumn{4}{l}{Adaptive on top (R4$-$R3): $+0.21$~pp, $p=0.30$\;$^{\dagger}$}\\
\bottomrule
\end{tabular}
\end{table}

We report the adaptive pillar's null result openly rather than tuning it away.
CAPN converges and is stable, and it never hurts; it simply does not add
separable signal on this single-layer backbone, where the label-aware distance it
reweights is already close to what the heuristic bandit finds. This is
consistent with the wider picture of Section~\ref{sec:generality}, where the
returns to any additional signal shrink as the backbone strengthens. We read it as
evidence that the learned policy needs a backbone with more filtering headroom
than CARE-GNN offers to express its advantage---which we return to below.

To test whether this null reflects the backbone's limited \emph{capacity} rather
than the policy itself, we repeated the full ablation (five matched seeds) on a
higher-capacity variant of the same backbone (a nonlinear feature encoder,
ego/neighbour separation, and a cosine schedule). The pattern sharpens on three
fronts. First, the semantic pillar's gain \emph{grows} with capacity: GNN+LLM
reaches AUC $0.7952 \pm 0.0042$ ($+2.96$~pp over the upgraded backbone,
$p<0.01$), and the full framework $0.7951 \pm 0.0032$ ($+2.94$~pp, $p<0.01$).
Second, the adaptive pillar stays within noise in both directions ($+0.46$~pp
alone, $p=0.18$; $-0.02$~pp on top of the LLM, $p=0.95$). Third---and this we
...
\emph{kind} therefore does not give the policy separable decisions to make; the
limiter is the backbone's single-threshold \emph{selection mechanism}---which
points not at a bigger CARE-GNN but at backbones whose neighbour selection is
intrinsically richer.
...
most stable gain; the learned policy is directionally positive alone but not
significant, and does not help on top of the text signal.}
\label{tab:wrapper}
\begin{tabular}{lccc}
\toprule
...
worst seed rises from $0.737$ to $0.810$). The adaptive pillar's solo
contribution is its largest yet ($+1.20$~pp, halving the variance), and the
progression of that solo effect across backbones---$+0.19$~pp on CARE-GNN,
$+0.46$~pp on its higher-capacity variant, $+1.20$~pp here---is exactly the
monotone trend the selection-headroom hypothesis predicts. But it is not
statistically significant on any of the three ($p=0.63$ here): per-seed, the
policy wins on the seeds where the native sampler struggles and loses on the
seed where it excels, and at $n=5$ that inconsistency swamps the mean. Stacked
on top of the semantic signal, the policy again fails to add ($-1.46$~pp,
n.s.) and reintroduces the very instability the gate had removed (one seed
collapses to $0.72$). The wrapper experiment thus establishes the framework's
portability while sharpening its internal attribution: the framework transfers
to a new backbone with only its state features and actuation point rewritten,
its semantic pathway delivers the decisive gain wherever it goes, and its
adaptive pathway---under the most favourable conditions we could construct for
it---remains directionally promising but statistically unproven.
...
(Table~\ref{tab:portable}). And the end-to-end framework, at $0.7838$ AUC on this
...
of a feature concatenation. The adaptive pathway is the framework's research
frontier: its solo effect strengthens monotonically with the host's selection
richness, and making it clear significance will take variance-control
machinery (clipped or trust-region updates, more seeds, stronger critics) and
imbalance-aware hosts such as PC-GNN---future work the wrapper now makes
cheap to run.
...
contributes rather than assuming it. The framework delivers: it significantly
outperforms its structural base on both backbone variants we tested
($+1.90$ and $+2.94$~pp, $p<0.01$), transfers to a second selection-based
...
plainly: the semantic pillar drives today's gains; the adaptive pillar's solo
effect grows monotonically with the host's selection richness, exactly as the
design predicts, but does not yet clear seed noise. A unified framework whose
components can be measured, detached, and re-hosted is precisely what makes
that honest accounting---and its continued improvement---possible; making the
adaptive pillar's promise statistical fact is the framework's next step.
