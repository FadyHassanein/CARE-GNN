...
\setabstract{%
  Graph neural networks are a standard substrate for fraud detection on multi-relation review graphs, yet sophisticated fraudsters evade neighbourhood aggregation by camouflaging both node attributes and connections. Existing defences are partial: reinforcement-learning approaches such as CARE-GNN rely on heuristic bandits with limited per-node adaptability and no external signal, while recent GNN--LLM integrations enrich node features but lack a learned policy for dynamic relation weighting. We propose a three-pillar framework unifying structural, adaptive, and semantic reasoning under a single joint objective. The structural pillar is a multi-relation CARE-GNN backbone; the adaptive pillar, the Camouflage-Aware Policy Network (CAPN), replaces the heuristic bandit with an actor--critic policy that learns per-node, per-relation filtering thresholds; the semantic pillar injects LLM-derived reasoning scores into the policy state and fraud-specific textual risk scores into node features via a dimensionality-preserving gated residual.
  On YelpChi, validated across twenty held-out seeds, the full framework improves on its structural base by $+2.70$~pp AUC ($20/20$ seeds, $p<10^{-16}$). Pillar-wise attribution shows the semantic and adaptive pillars each contribute significantly and, critically, compose when stacked ($+0.74$~pp from the policy on top of the semantic signal). We report the method's boundaries transparently: on a high-variance backbone the adaptive effect is statistically undetectable, and our LLM-prior initialisation initially suppressed the policy until de-confounded. Two studies establish generality: the semantic signal transfers to eight backbones---significantly improving six, including the benchmark-leading XGB-Graph---and the framework attaches unchanged, as a backbone-agnostic wrapper, to a second selection-based backbone. Finally, the semantic pillar's value lies in its six fraud-specific score dimensions, not the model computing them: a zero-cost heuristic scorer is statistically indistinguishable from the LLM, making the pillar deployable without API dependence.
}
