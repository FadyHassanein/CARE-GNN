...
\setabstract{%
  ...
  On YelpChi, validated across twenty held-out seeds, the full framework improves on its structural base by $+2.70$~pp AUC ($20/20$ seeds, $p<10^{-16}$). Pillar-wise attribution shows the semantic and adaptive pillars each contribute significantly and, critically, compose when stacked ($+0.74$~pp from the policy on top of the semantic signal). Two studies establish generality: the semantic signal transfers to eight backbones---significantly improving six, including the benchmark-leading XGB-Graph---and the framework attaches unchanged, as a backbone-agnostic wrapper, to a second selection-based backbone.
}
